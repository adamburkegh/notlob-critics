...
- **Dominic Fox wrote the first external notlob project**, `patches-dsp` —
  the first known notlob project authored by someone other than the tool's
  author. Committed 27 May 2026 (Fox's git history); sent to the notlob
  author by email 28 May 2026. It is on Fox's `rust-bindings` fork
  (github.com/poetix/notlob), carries a Rust binding of his own making, and
  renders a slice of an audio-DSP kernel library. Reviewed in full — see
  `teri/patches-dsp.md`. It is the strongest project in the corpus, and the
  one corpus entry with no selection pressure from the tool's designer: the
  most evidentially valuable kind of sample. Notable that a `~property`
  test *generated by the notlob workflow during the port* (per Fox's email,
  27–28 May 2026) caught a real instability in the filter, arising from a
  bug in a clamping calculation — the author had not derived the invariant
  by hand. The claim layer's thesis demonstrated in the wild by an external
  author, and demonstrated in its stronger form: the format produced the
  checking artefact, not merely hosted a human's. Provenance of the prose
  itself: Fox's email implies the agent drafted the "nice flowing
  explanations"; the domain judgement is his, the keystrokes likely the
  agent's, steered. Recorded as human-machine collaboration, not sole
  authorship either way. To be added to the registry once a draft paper
  exists as a landing place.
